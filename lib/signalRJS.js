...
SignalRJS.prototype.hubRoute = function(req,res){
	...
	hubs.parseMessage(req,function(responseMsg,toUser){
		...
		res.json({
			H: data.H, // Hub name
			C: Date.now(), // Time when run
			M: data.M // Method name
		})
		if(toUser)
...
SignalRJS.prototype.broadcastToConnectionTokens = function(messageData, connectionTokens) {
	var self = this;
	this._connectionManager.forEach(function(connection){
		if(connectionTokens.find(token => connection.connection.req.signalrjs.token) !== undefined) {
			var transport = self._transports[connection.type];
			if(transport)
				transport.send(connection.connection,messageData);
		}
...
SignalRJS.prototype.createListener = function(){
	...
	app.use(self.route+'/abort', function(req,res){
		self.abort(req,res);
		self.emit(EVENT_CONSTANTS.disconnected, req.signalrjs);
	});
